-- Fix existing logs with 0 calories: estimate based on sets/reps from workout_items
UPDATE workout_logs SET calories_estimated = GREATEST(
  ROUND((COALESCE(duration_seconds, 30)::numeric / 60) * 8),
  9 -- minimum 9 kcal per exercise (3 sets * 3 kcal)
)
WHERE calories_estimated = 0 OR calories_estimated IS NULL;

-- Update calculate_daily_metrics to ensure minimum calorie estimation
  ...
  SELECT
    COALESCE(SUM(
      GREATEST(
        ...
        ELSE ROUND((GREATEST(wl.duration_seconds, 30)::numeric / 60) * 8) END,
        9
      )
    ), 0),
